...
jQuery(document).on('action.init_shortcodes', function (e, container) {
    ...
    function trx_addons_detect_active_toc() {
        ...
        toc_menu_items.each(function (idx) {
            ...
            var off_next = id_next && idx < toc_menu_items.length - 1 ? parseInt(jQuery(id_next).offset().top, 10) : 1000000;
            var scroll_offset = parseInt(jQuery(window).scrollTop(), 10);
...
jQuery(document).on('action.ready_trx_addons', function () {
    ...
    if (jQuery('.sc_layouts_cart').length > 0) {
        jQuery('body:not(.added_to_cart_inited)').addClass('added_to_cart_inited').bind('added_to_cart', function () {
            ...
            jQuery('.sc_layouts_cart').data({'items': cnt ? cnt : 0, 'summa': total ? total : 0});
        });
    ...
    function trx_addons_menu_collapse() {
        jQuery('.sc_layouts_menu').each(function () {
            ...
            sc_layouts_item.siblings().each(function () {
                if (!jQuery(this).hasClass('vc_empty_space')) w_max -= (jQuery(this).outerWidth() + parseFloat(jQuery(this).css('marginLeft')) + parseFloat(jQuery(this).css('marginRight')));
            });
